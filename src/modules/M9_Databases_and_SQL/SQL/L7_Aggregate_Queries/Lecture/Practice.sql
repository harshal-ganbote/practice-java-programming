...
--
--
-- Find out how many distinct rental rates were there for movies
--  which had rating "PG-13"
...
--
--
-- Total Orders per customer, customer_id, customer_name,
-- number of orders this customer has made
SELECT customer_id,
  COUNT(*)
FROM rental
...
-- GROUP BY --> select you can select aggregate 
-- value along with one or more cols given in group 
-- clause, or they are function of the given group by column
SELECT customer_id,
  first_name,
  email,
...
HAVING COUNT(*) > 200;
SELECT rating,
  COUNT(*) AS rating_per_movies
FROM film
GROUP BY rating
HAVING rating_per_movies >= 200;
--
--
-- Query 2: Sakila DB
...
SELECT CONCAT(c.first_name, " ", c.last_name),
  COUNT(*) AS customer_rental
FROM rental r
  JOIN customer c USING (customer_id)
GROUP BY customer_id
HAVING customer_rental >= 30
